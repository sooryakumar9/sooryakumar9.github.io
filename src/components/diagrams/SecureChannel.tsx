...
const READOUT: Record<Mode, { captured: string; note: string[] }> = {
  rsa: {
    ...
    note: [
      "RSA-2048 handshake recorded.",
      "Factorable by Shor's algorithm on a",
      "large quantum computer. Harvest now,",
      "decrypt later.",
    ],
  },
  pq: {
    captured: "▚▒▞▓░▚▒█▞░▓▒▚▞▒░▓█▚▒▞░",
    note: [
      "Post-quantum key exchange recorded.",
      "Recorded traffic stays noise — even",
    ...
    <div>
      ...
      <svg
        ...
        aria-label={`Encrypted channel between sender and receiver with an interceptor tapping the line. Current mode ${
          mode === "rsa" ? "RSA, interceptor recovers plaintext in time" : "post-quantum, interceptor records only noise"
        }`}
      >
        {/* endpoints + channel */}
